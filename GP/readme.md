...
Отсюда правило: `gp_timeout` поднимать можно только вместе с серверным лимитом
(`gp_server_limit`) и всегда держать его на пять минут ниже — санитар монитора это
проверяет. `wf_timeout` отдельного воркфлоу не урезается, но выше лимита даёт
предупреждение в заметке. Задрать наш таймаут выше
...
трогает, а не потому, что он верен. Поэтому годность меряется сверкой с источником —
её делает `.claude/scripts/check_context.py` на каждый `pull`, `merge` и `checkout`
(хуки в `.githooks/`), отвечая на два разных вопроса:

| Что сказано | Что случилось | Что делать |
|---|---|---|
| «отличается от `<ревизия>` — снимок правили здесь» | кто-то поправил копию | перенести правку в `HR_Data`, копию вернуть |
| «соответствует текущему `HR_Data`, но не записанной ревизии» | врёт `source.json` | обновить: `sync_context.py --gp` |
| «источник ушёл вперёд на N коммитов по этим объектам» | снимок устарел | то же |
| «чекаута `~/HR_Data` нет — снимок не сверялся» | сверка не проводилась | ничего; «не проверено» ≠ «хорошо» |

Обновление:

```bash
python3 .claude/scripts/sync_context.py --gp
```

Скрипт берёт файлы из `HEAD` чекаута (не из рабочего дерева: записанная ревизия должна
соответствовать содержимому) и переписывает `source.json`. Набор файлов он **не
расширяет** — переносит ровно то, что здесь уже лежит: список объектов курируется
руками, скрипт лишь не даёт ему протухнуть. Нужен новый объект — скопировать один раз
руками, дальше он подхватится.

Структура повторяет `HR_Data`, только без префикса `s_grnplm_vd_hr_edp_` в имени схемы.
Зеркальные `drop`-скрипты не копируются: полная сигнатура и так стоит первой строкой
каждого файла.

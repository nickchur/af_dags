...
_BASE_WHERE = {
    ...
    # min=1, max=314823, count=314823, порядок совпадает). Граница — МАКСИМУМ id среди уже
    # старых, а не минимум среди свежих: задача, начатая 40 дней назад и закончившаяся 5
    # дней назад, имеет маленький id и свежий date_done — она утянула бы минимум вниз и
    # застопорила чистку. Строка, вставленная недавно, старый date_done иметь не может,
    # поэтому максимум среди старых стоит ровно на границе cutoff.
    #
    # Нет ни одной старой строки — подзапрос даёт NULL, сравнение ложно, не удаляется
    # ничего. Это и защищает живую STARTED-строку работающей задачи: у неё date_done пуст,
    # и без границы по id её снесло бы вместе с осадком.
    #
    ...
    # Проверено на стенде откатом: старая сирота (подделанная из существующей строки) под
    # удаление попадает, а обе живые STARTED остаются; условие на живой строке именно NULL.
    'celery_taskmeta': (
        '({col} < :cutoff OR ({col} IS NULL AND {p}id <= '
        '(SELECT MAX(id) FROM main.celery_taskmeta WHERE date_done < :cutoff)))'
    ),
}

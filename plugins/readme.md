...
*2026-09-17 09:26 MSK · v1.5 · Nick Churkin · [NSChurkin@sber.ru](mailto:NSChurkin@sber.ru)*
...
- **Контур**: альфа или сигма, от этого зависит топик отправки — `TFS.PKAPHR.IN` против
  `TFS.HRPLT.IN`. Признак альфы — **само наличие `ENVIRONMENT`**, а не её значение: на
  альфе платформа переменную задаёт, на сигме нет
...
Поэтому состояний три, и различает их `pg_stat_activity`:

| Журнал | Серверный процесс | Что делает `ctl_exe_recover` |
|---|---|---|
| ответ есть | — | отдаёт его: `('ok', ответ)` |
| ответа нет | занят нашей процедурой | ждёт завершения, потом отдаёт ответ |
| ответа нет | свободен | `('fail', …)` — транзакция откачена |
| — | определить нечем | `('fail', …)` — ETL заново не запускаем |

«Определить нечем» — это неизвестный pid, недоступный Greenplum или исчерпанный потолок
ожидания. Во всех трёх случаях падаем: объявить работу несостоявшейся, пока она идёт,
значит отдать загрузку на повтор в CTL и получить второй ETL параллельно первому.
...
Тракт ТФС выбирает топик по контуру, и признаком служит **наличие переменной окружения
`ENVIRONMENT`**: есть — альфа, нет — сигма.
...
celery не собирается. Поэтому на альфа-стенде `ENV_SPACE` выставлен в `sigma` намеренно,
ради библиотеки, и описанием контура он больше не является.

`ENV_STAND` для этого тоже не годится: `DEV` и `PROM` есть у обоих контуров, и по ней
альфу от сигмы не отличить. Она отвечает на другой вопрос — стенд или пром, — и её
разбирает `utils.env_stand`.

...
def s3_base() -> str:
    ...
    Префикс логов (часть после бакета) отбрасывается: тракт живёт в корне бакета, а не внутри
    папки логов. Так его видно с первого взгляда, и правило чистки, написанное для логов, его
    не накрывает — фильтр правила жизненного цикла в S3 работает по префиксу, исключений в нём
    нет. Там же лежат снимки дагов и журналы отбивок воркеров: в корне — всё, что не логи.

    До 2026-08-15 тракт жил под префиксом логов, и переключение было константой
    S3_UNDER_LOG_PREFIX. Константу убрали 2026-09-16: за месяц её ни разу не двигали, а
    переключение меняет ПУТЬ К ДАННЫМ — очередь и квитанции по новому пути не находятся, файл
    не уезжает, а wait_confirm висит до таймаута. Переносить тракт, если когда-нибудь
    понадобится, безопаснее осознанным переносом ключей, а не правкой константы.
    """
    ...
    return f"{conn}://{bucket}/{S3_PREFIX}"

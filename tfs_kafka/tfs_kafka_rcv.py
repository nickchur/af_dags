...
* пришла хоть одна квитанция → таск **зелёный**, сами квитанции в XCom записью на сценарий, итоги окна по сценариям — в `return_value`;
...
(`tools/test_kafka.py`). На этих топиках его запускать нельзя.
...
# Упор в него — не штатная ситуация, а сигнал: опрос предупреждает об этом в лог, иначе
# растущий лаг выглядел бы как обычная успешная работа.
MAX_MESSAGES = 5000
...
def tfs_kafka_rcv_dag():
    ...
    def _push_xcom(context, items: list) -> None:
        """📤 Квитанции окна в XCom: запись на СЦЕНАРИЙ, внутри {метка времени: квитанция}.

        Топик квитанций общий на все маршруты ТФС, поэтому в одном ране соседствуют чужие
        друг другу пакеты (ЕР и xStream). Ключ по сценарию разводит их сразу, без чтения
        значений.

        Метка времени — ключом внутреннего словаря, а не XCom: отдельные записи на
        квитанцию UI Airflow 2.10 показывал как «Invalid input», хотя данные были целы.
        API отдаёт значение питоновским repr словаря одинаково и для них, и для
        return_value, который рисуется деревом (проверено curl-ом на стенде: ключ
        с пробелом читается и отвечает 200). Значит спотыкается сам просмотрщик, и спорить
        с ним дешевле формой записи, чем содержимым.

        Записи обновляются на КАЖДОМ опросе: XCom таска Airflow чистит перед каждым
        исполнением, а в reschedule опрос — отдельное исполнение.
        """
        by_scenario: dict[str, dict] = {}
        for item in items:
            # Сценария нет у неразобранной квитанции (status_code < 0) — ей своя запись,
            # иначе она потерялась бы среди чужих или осталась без ключа вовсе.
            key = item['row'].get('scenario_id') or 'unknown'
            by_scenario.setdefault(key, {})[item['key']] = item['row']

        for scenario, receipts in by_scenario.items():
            context['ti'].xcom_push(key=scenario, value=receipts)
    ...
    def poke_topic(topic: str, **context) -> PokeReturnValue:
        ...
        try:
            ...
            # Упёрлись в потолок опроса — значит топик вычерпан не до конца и лаг растёт.
            # Молчать тут нельзя: снаружи это выглядит как обычный успешный опрос, а на
            # деле остаток догоняется по 5000 за раз и может не догнаться вовсе.
            if len(rows) >= MAX_MESSAGES:
                logger.warning("🚨 %s: за опрос набрано %d сообщений — потолок MAX_MESSAGES. "
                               "Топик вычерпан не полностью, остаток уедет следующим опросом; "
                               "если так повторяется — смотреть лаг группы", topic, len(rows))
        ...
        if elapsed >= WINDOW and seen:
            ...
            for r in rows:
                stat = scenarios.setdefault(r.get('scenario_id') or 'unknown',
                                            {'count': 0, 'failed': 0, 'unknown': 0})
                stat['count'] += 1
                stat['failed'] += r['status_code'] > 0
                stat['unknown'] += r['status_code'] < 0
